...
static pte_t *walkpgdir(pde_t *pgdir, const void *va, int create) {
  ...
  if (*pde & PTE_P) {
    ...
    if (!create || (pgtab = (pte_t *)kalloc()) == 0) return 0;
...
int copy_mmap_pgdir(pde_t *pgdir, struct mmap_area *mmapArea) {
  ...
  for (uint i = mmapArea->start_addr; i < mmapArea->end_addr; i += PGSIZE) {
    ...
    if ((mmapArea->flags & MAP_SHARED) == MAP_SHARED) {
      ...
      if (mappages(pgdir, (void *)i, PGSIZE, PTE_ADDR(*pte), PTE_W | PTE_U) <
          0) {
      ...
      cprintf("map private\n");
      if (!(*pte & PTE_P)) panic("copy_mmap_pgdr: page not present");
...
// Helper functions to check memory region availability
int memoryRegionAvailable(struct proc *p, int addr, size_t length) {
  for (int i = 0; i < 32; i++) {
    if (p->mmap_list[i].valid == 0) {
      continue;
    }

    int start_addr = p->mmap_list[i].start_addr;
    int end_addr = p->mmap_list[i].end_addr;

    // Check if requested address overlaps with allocated region
    if ((addr >= start_addr && addr < end_addr) ||
        (addr + length > start_addr && addr + length <= end_addr)) {
...
int getMemoryRegion(struct proc *p, int addr, size_t length) {
  for (int i = 0; i < 32; i++) {
    if (p->mmap_list[i].valid == 0) {
      continue;
    }
    ...
    if ((addr >= start_addr && addr < end_addr) ||
        (addr + length > start_addr && addr + length <= end_addr)) {
      return i;  // Region found
    }
  }
  return -1;  // Not found
}

// Helper function to allocate and map memory
char *kalloc_and_map(struct proc *p, void *addr, size_t length) {
  char *mem = kalloc();  // Call kalloc to allocate memory
  if (mem == 0) {
    cprintf("allocuvm out of memory\n");
    return (void *)-1;
  }
  memset(mem, 0, PGSIZE);  // Clear allocated memory
...
void *mmap(void *addr, size_t length, int prot, int flags, int fd, int offset) {
  ...
  if ((flags & MAP_FIXED) != 8) {
    ...
    p->mmap_free_addr =
        u_addr +
        PGROUNDUP(length);  // probably need to check that next VA is free
  }
...
int munmap(void *addr, size_t length) {
  ...
  pte_t *pte;
  for (int page_base = u_addr; page_base < u_addr + length;
       page_base += PGSIZE) {
  ...
  if (mmapArea->start_addr + PGROUNDUP(length) >=
      mmapArea->end_addr) {  // if length is >= mapped address range, clear
                             // start and end
    ...
    mmapArea->start_addr =
        mmapArea->start_addr +
        PGROUNDUP(length);  // else we just need to reset the start addr
  }
  mmapArea->flags = 0;
  mmapArea->fd = 0;
  p->mmap_list[i].valid = 0;
